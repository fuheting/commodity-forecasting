...
from commodity_forecasting.evaluation import (
    EvaluationConfig,
    EvaluationInputError,
    EvaluationResults,
    EvaluationRow,
    calculate_evaluation_results as calculate_configured_evaluation_results,
    calculate_group_result as calculate_configured_group_result,
    calculate_interval_metrics as calculate_configured_interval_metrics,
    calculate_point_metrics,
    calculate_quantile_metrics as calculate_configured_quantile_metrics,
    describe_evaluation_contract,
    empirical_quantile_coverage,
    interval_metrics,
    mean_absolute_error,
    mean_pinball_loss,
    normalize_evaluation_rows as normalize_configured_evaluation_rows,
    parse_evaluation_csv as parse_configured_evaluation_csv,
    pinball_loss,
    root_mean_squared_error,
)

from . import backtest_publication as rolling_origin
...
EVALUATION_CONFIG = EvaluationConfig(
    source_columns=rolling_origin.OUTPUT_COLUMNS,
    quantile_columns=QUANTILE_COLUMNS,
    quantiles=QUANTILES,
    horizon_steps=HORIZON_STEPS,
    expected_origin_count=ORIGIN_COUNT,
    interval_level=INTERVAL_LEVEL,
    interval_lower_column="interval_80_lower",
    interval_upper_column="interval_80_upper",
    provenance_fields=(
        "reference_model_id",
        "publication_label",
        "publication_proxy",
...
class PublicationError(EvaluationError):
    ...
    def __init__(self, message: str, *, invalidation_error: Exception | None = None) -> None:
...
def normalize_evaluation_rows(rows: Sequence[Mapping[str, object]]) -> tuple[EvaluationRow, ...]:
    return normalize_configured_evaluation_rows(rows, config=EVALUATION_CONFIG)
...
def parse_evaluation_csv(payload: str | bytes) -> tuple[EvaluationRow, ...]:
    ...
    return parse_configured_evaluation_csv(payload, config=EVALUATION_CONFIG)


def calculate_interval_metrics(rows: Sequence[EvaluationRow]) -> dict[str, object]:
    return dict(calculate_configured_interval_metrics(rows, config=EVALUATION_CONFIG))


def calculate_quantile_metrics(rows: Sequence[EvaluationRow]) -> list[dict[str, float]]:
    return [
        cast(dict[str, float], dict(record))
        for record in calculate_configured_quantile_metrics(rows, config=EVALUATION_CONFIG)
    ]
...
def calculate_group_result(
    ...
    return dict(
        calculate_configured_group_result(
            rows, group_type=group_type, group_value=group_value, config=EVALUATION_CONFIG
        )
    )


def calculate_evaluation_results(rows: Sequence[EvaluationRow]) -> EvaluationResults:
    """Return aggregate then ordered horizon 1/2/3 results."""

    return calculate_configured_evaluation_results(rows, config=EVALUATION_CONFIG)


def evaluation_contract() -> dict[str, object]:
    return describe_evaluation_contract(EVALUATION_CONFIG)
...
def _diagnostic(classification: str, stage: str, exc: Exception) -> dict[str, object]:
...
def build_source_binding(
    ...
    if rows[0].provenance_value("reference_model_id") != model_id:
    ...
    binding: dict[str, object] = {
        ...
        "p1_06_publication_label": rows[0].provenance_value("publication_label"),
        "p1_06_publication_proxy": rows[0].provenance_value("publication_proxy"),
        "p1_06_vintage_limitation": rows[0].provenance_value("vintage_limitation"),
    ...
    try:
        ...
        try:
            ...
            raise PublicationError(
                "evaluation publication failed; invalid marker publication also failed: "
                f"{type(invalidation_exc).__name__}",
                invalidation_error=invalidation_exc,
            ) from exc
