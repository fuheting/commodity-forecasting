...
def _installed_package_path() -> Path:
    package = importlib.import_module("commodity_forecasting")
    package_file = getattr(package, "__file__", None)
    if not package_file:
        raise RepoRootError("installed commodity_forecasting package has no filesystem location")
...
def _child_probe(repo_root: Path) -> dict[str, Any]:
    ...
    installed_outside_repo = not _is_relative_to(_installed_package_path(), root)
...
def _base_record(repo_root: Path) -> dict[str, Any]:
    ...
    expected_pythonpath = os.pathsep.join((str(repo_root / "src"), str(repo_root)))
    host_bootstrap_explicit = os.environ.get("PYTHONPATH") == expected_pythonpath
    host_pythonpath_state = "explicit_repo_paths" if host_bootstrap_explicit else "missing_or_mismatch"
    ...
    return {
        ...
        "sanitized_command": (
            ...
            "tools.workflows.dependency_readiness verify-clean-install "
...
def verify_clean_install(
    ...
    *,
    ...
    if enforce_host_bootstrap and not record["checks"]["host_bootstrap_explicit"]:
        ...
        record["errors"] = ["host bootstrap must set PYTHONPATH to the explicit repository src and tooling roots"]
