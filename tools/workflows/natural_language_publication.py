"""Evidence schema, publication, and roadmap orchestration for P1-08."""
...
from commodity_forecasting.analysis import (
    AnalysisOutputError,
    extract_message_history as extract_analysis_message_history,
    query_anchors_present as analysis_query_anchors_present,
)

from . import (
    backtest_publication as rolling_origin,
    evaluation_publication as evaluation,
    target_publication as target_pipeline,
)
from .target_publication import parse_target_csv
...
class OutputContractError(NaturalLanguageError):
    """Raised when an agent result violates the P1-08 output contract."""
...
def fallback_source_binding(root: Path) -> dict[str, str]:
...
query_anchors_present = partial(
    analysis_query_anchors_present,
    required_anchors=REQUIRED_QUERY_ANCHORS,
)
...
def extract_message_history(result: object) -> dict[str, object]:
    try:
        return dict(
            extract_analysis_message_history(
                result,
                expected_provider="deepseek",
                expected_model="deepseek-v4-flash",
                structured_output_tool=DEFAULT_OUTPUT_TOOL_NAME,
            )
        )
