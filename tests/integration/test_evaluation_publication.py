...
from commodity_forecasting import evaluation as evaluation_core
from tools.workflows import backtest_publication as rolling_origin
from tools.workflows import evaluation_publication as evaluation
...
def _published(root: Path) -> dict[str, Any]:
    return cast(
        dict[str, Any],
        evaluation.publish_evaluation_bundle(
...
def test_reusable_core_uses_explicit_non_phase_configuration() -> None:
    columns = (
        "origin",
    ...
    config = evaluation_core.EvaluationConfig(
        source_columns=columns,
        quantile_columns=("q25", "q75"),
        quantiles=(0.25, 0.75),
        horizon_steps=(1, 2),
        expected_origin_count=2,
        interval_level=90,
        interval_lower_column="interval_90_lower",
        interval_upper_column="interval_90_upper",
        provenance_fields=("model",),
    )
    rows = [
        {
            "origin": origin,
            ...
            "actual": 2.0,
            "point_forecast": 1.0,
            "interval_90_lower": 0.0,
            "interval_90_upper": 3.0,
            "q25": 1.0,
            "q75": 3.0,
            "model": "configured-model",
        }
        for origin in ("2026-01-01", "2026-02-01")
        for horizon in (1, 2)
    ]

    normalized = evaluation_core.normalize_evaluation_rows(rows, config=config)
    results = evaluation_core.calculate_evaluation_results(normalized, config=config)

    assert results["aggregate"]["row_count"] == 4
    assert results["aggregate"]["interval"]["level"] == 90
    assert [group["group_value"] for group in results["per_horizon"]] == [1, 2]
    assert evaluation_core.describe_evaluation_contract(config)["source_row_count"] == 4
...
def test_closed_schema_rejects_unknown_and_decision_fields() -> None:
    ...
    mutated = cast(dict[str, Any], copy.deepcopy(evidence))
...
def test_invalid_marker_write_failure_is_reported_with_original_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = _publication_root(tmp_path)
    original_replace_marker = evaluation._replace_json_marker

    def fail_invalid_marker(
        repo_root: Path, evidence: Mapping[str, object], replace_file: object = evaluation.os.replace
    ) -> None:
        protocol = cast(Mapping[str, object], evidence["publication_protocol"])
        if protocol["marker_state"] == "invalid_final":
            raise OSError("simulated invalid marker write failure")
        original_replace_marker(repo_root, evidence, replace_file)

    def fail_source(
        repo_root: Path,
    ) -> tuple[Mapping[str, object], tuple[evaluation.EvaluationRow, ...], dict[str, object]]:
        raise ValueError("simulated source failure")

    monkeypatch.setattr(evaluation, "_replace_json_marker", fail_invalid_marker)
    monkeypatch.setattr(evaluation, "_load_validated_source", fail_source)

    with pytest.raises(
        evaluation.PublicationError,
        match="invalid marker publication also failed: OSError",
    ) as raised:
        evaluation.publish_evaluation_bundle(root)

    assert isinstance(raised.value.__cause__, ValueError)
    assert str(raised.value.__cause__) == "simulated source failure"
    assert isinstance(raised.value.invalidation_error, OSError)
    assert str(raised.value.invalidation_error) == "simulated invalid marker write failure"
    marker = _read_marker(root)
    assert marker["publication_protocol"]["marker_state"] == "invalid_in_progress"


def test_interrupt_is_not_converted_to_publication_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = _publication_root(tmp_path)
    ...
    monkeypatch.setattr(evaluation, "_load_validated_source", interrupt_source)
