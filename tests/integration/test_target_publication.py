...
from tools.workflows import target_publication as target_pipeline
from tools.workflows.readiness_contract import (
...
from tools.workflows.dependency_readiness import sha256_file
from tools.workflows.target_publication import (
    CSV_FIELDS,
...
def test_core_workbook_reader_uses_explicit_source_configuration(tmp_path: Path) -> None:
    path = tmp_path / "configured-source.xlsx"
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Configured Sheet"
    sheet.append(("Period", "Configured Target"))
    sheet.append(("2024M01", 1.25))
    sheet.append(("2024M02", 1.5))
    workbook.save(path)
    workbook.close()

    contract = MonthlyTargetContract(
        unique_id="configured_target",
        period_start="2024M01",
        period_end="2024M02",
        period_count=2,
    )
    source = WorkbookTargetSource(
        worksheet_name="Configured Sheet",
        target_column="Configured Target",
        expected_sha256=sha256_file(path),
        target=contract,
    )

    observation = extract_workbook_target(path, source)
    encoded = serialize_target_csv(observation.rows)

    assert observation.header_row == 1
    assert observation.target_column_index == 2
    assert [row.y for row in observation.rows] == ["1.25", "1.5"]
    assert parse_target_csv_bytes(encoded, contract) == observation.rows
    ...
    for root in roots:
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "tools.workflows.target_publication",
                "publish",
                "--repo-root",
                str(root.resolve()),
            ],
