...
from tools.workflows import dependency_readiness as readiness
...
from tools.workflows.readiness_evidence import (
    EvidenceError,
...
from tools.workflows.repository_paths import (
...
from tools.workflows.dependency_readiness import (
    CommandResult,
...
def _valid_record(*, classification: str = "pass") -> dict[str, object]:
    ...
    record: dict[str, object] = {
        ...
        "sanitized_command": "python -m tools.workflows.dependency_readiness verify-clean-install --repo-root /repo",
        "repo_root": "/repo",
...
def test_invalid_child_pass_is_published_as_fail_instead_of_leaving_stale_pass(
    ...
    monkeypatch.setenv(
        "PYTHONPATH",
        os.pathsep.join((str(tmp_path / "src"), str(tmp_path))),
    )
    temporary_root = tmp_path / "controller"
...
def test_host_bootstrap_starts_from_uninstalled_src_checkout(tmp_path: Path) -> None:
    ...
    shutil.copytree(REPO_ROOT / "tools", repository / "tools")
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.workflows.dependency_readiness",
    ...
    assert evidence["host_pythonpath_state"] == "explicit_repo_paths"
    assert evidence["sanitized_command"].startswith(
        f"PYTHONPATH={repository / 'src'}{os.pathsep}{repository} {sys.executable}"
    )
