...
from tools.workflows.target_publication import MODEL_READY_RELATIVE_PATH, parse_target_csv
...
def test_timecopilot_bridge_implements_forecast_protocol_with_exact_poc_config() -> None:
    config = rolling_origin.POC_TIMECOPILOT_CONFIG
    adapter = TimeCopilotForecastAdapter(object(), config)

    assert isinstance(adapter, rolling_origin.ForecastOnlyAdapter)
    assert config.model_id == "autogluon/chronos-2-small"
    assert config.model_alias == "P105"
    assert config.adapter_class == "timecopilot.models.foundation.chronos.Chronos"
    assert config.package_distributions == (
        "timecopilot",
...
def test_package_and_integration_import_without_optional_runtime_dependencies() -> None:
    script = f"""
...
class RejectOptionalRuntime(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        if fullname.split('.')[0] in {{'pandas', 'timecopilot'}}:
            raise ImportError(f'blocked optional import: {{fullname}}')
        return None

sys.meta_path.insert(0, RejectOptionalRuntime())
import commodity_forecasting
import commodity_forecasting.integrations.timecopilot
assert 'pandas' not in sys.modules
assert 'timecopilot' not in sys.modules
"""
